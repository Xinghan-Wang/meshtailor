# v13paper100k full 10k test evaluation (p0 protocol).
...
$p0out = "$root\test_outputs_v13paper100k_p0_full"
...
if ($ngt -lt 10000) {
    ...
    $proc0 = Start-Process -FilePath $py -ArgumentList $argstr0 -NoNewWindow -Wait -PassThru `
        -RedirectStandardOutput "$logd\v13_full10k_gt_log.txt" `
        -RedirectStandardError "$logd\v13_full10k_gt_err.txt"
...
$proc = Start-Process -FilePath $py -ArgumentList $argstr -NoNewWindow -Wait -PassThru `
    -RedirectStandardOutput "$logd\v13_full10k_infer_log.txt" `
    -RedirectStandardError "$logd\v13_full10k_infer_err.txt"
...
$proc2 = Start-Process -FilePath $py -ArgumentList $argstr2 -NoNewWindow -Wait -PassThru `
    -RedirectStandardOutput "$logd\v13_full10k_eval_log.txt" `
    -RedirectStandardError "$logd\v13_full10k_eval_err.txt"
...
$argstr3 = ('"' + "$root\tools\finalize_v10fix_10k.py" + '"' +
    " --model_dir `"$p0out`" --gt_dir `"$gtout`"" +
    " --data_dir `"$data`" --split_file `"$split`"" +
    " --summary `"$logd\v13_full10k_summary.txt`"")
$proc3 = Start-Process -FilePath $py -ArgumentList $argstr3 -NoNewWindow -Wait -PassThru `
    -RedirectStandardOutput "$logd\v13_full10k_finalize_log.txt" `
    -RedirectStandardError "$logd\v13_full10k_finalize_err.txt"
