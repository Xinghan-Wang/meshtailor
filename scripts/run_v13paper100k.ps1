# Final model training: v13 maximal (paper B.1) labels, paper protocol, 100k train split.
# Reproduce best_v13paper100k.pt. Hyperparameters identical to the paper-style run.
...
$proc = Start-Process -FilePath $py -ArgumentList $argstr -NoNewWindow -Wait -PassThru `
    -RedirectStandardOutput "$root\checkpoints\v13paper100k_log.txt" `
    -RedirectStandardError "$root\checkpoints\v13paper100k_err.txt"
