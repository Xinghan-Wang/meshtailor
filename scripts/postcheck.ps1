# Post-train structural quick-check of best_paper100k.pt on 150 test GIDs
...
$argstr = ('"' + "$root\tools\paired_protocol_eval.py" + '"' +
    " --legacy_ckpt `"$root\checkpoints\best_paper100k.pt`"" +
    " --paper_ckpt `"$root\checkpoints\best_paper100k.pt`"" +
    ...
    " --out_json `"$root\checkpoints\struct_150_paper100k.json`"")
...
$proc = Start-Process -FilePath $py -ArgumentList $argstr -NoNewWindow -Wait -PassThru `
    -RedirectStandardOutput "$root\checkpoints\struct_150_paper100k_log.txt" `
    -RedirectStandardError "$root\checkpoints\struct_150_paper100k_err.txt"
...
$argstr2 = ('"' + "$root\tools\oracle_start.py" + '"' +
    ...
    " --data_dir `"$root\processed_data_seamless_maximal`"" +
    " --split_file `"$root\meshtailor\data\split_seamless_26k.json`"" +
    " --n 150 --out `"$root\checkpoints\oracle_start_paper100k.txt`"")
...
$proc2 = Start-Process -FilePath $py -ArgumentList $argstr2 -NoNewWindow -Wait -PassThru `
    -RedirectStandardOutput "$root\checkpoints\oracle_start_paper100k_log.txt" `
    -RedirectStandardError "$root\checkpoints\oracle_start_paper100k_err.txt"
